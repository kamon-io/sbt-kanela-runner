...
// https://raw.githubusercontent.com/playframework/playframework/3.0.1/dev-mode/play-run-support/src/main/scala/play/runsupport/Reloader.scala
...
object KanelaReloader {
  ...
  def startDevMode(
      ...
      reloadCompile: () => Reloader.CompileResult,
      assetsClassLoader: ClassLoader => ClassLoader,
      monitoredFiles: Seq[File],
      fileWatchService: FileWatchService,
      generatedSourceHandlers: Map[String, Reloader.GeneratedSourceMapping],
      defaultHttpPort: Int,
      defaultHttpAddress: String,
      projectPath: File,
      devSettings: Seq[(String, String)],
...
import play.runsupport.KanelaReloader._
...
class KanelaReloader(
  ...
  @volatile private var currentSourceMap = Option.empty[Map[String, Reloader.Source]]
  ...
  def reload: AnyRef = {
    reloadLock.synchronized {
      if (changed || forceReloadNextTime || currentSourceMap.isEmpty || currentApplicationClassLoader.isEmpty) {
        ...
        // use KanelaReloader context ClassLoader to avoid ClassLoader leaks in sbt/scala-compiler threads
        KanelaReloader.withReloaderContextClassLoader {
          ...
          reloadCompile() match {
            ...
            case Reloader.CompileSuccess(sourceMap, classpath) =>
